...
def train_nlvr2(args, encoder, nlvr_config, model_config, tokenizer, device):
    ...
    # freeze the embeddings and bottom k layers
    for p in model.vilt_encoder.vilt.embeddings.parameters():
        p.requires_grad = False

    for i in range(9):
        for p in model.vilt_encoder.vilt.encoder.layer[i].parameters():
            p.requires_grad = False
